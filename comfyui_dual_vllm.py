"""
ComfyUI Dual VLLM Node (V5 - Dual Model, Synchronous)
======================================================
...
def load_presets():
    ...
    try:
        with open(presets_path, 'r', encoding='utf-8') as f:
            ...
            print(f"[ComfyUI Dual VLLM] Loaded {len(presets)} presets from {presets_path}")
            return presets
    except Exception as e:
        print(f"[ComfyUI Dual VLLM] Error loading presets: {e}")
...
def resize_image_smart(pil_image: Image.Image, target_width: int = 1024, target_height: int = 1024) -> Image.Image:
    ...
    if width == target_width and height == target_height:
        print(f"[ComfyUI Dual VLLM] Image already at target size: {width}x{height}")
        return pil_image

    print(f"[ComfyUI Dual VLLM] Resizing image: {width}x{height} → {target_width}x{target_height}")
...
def call_llm_api(
    ...
    # If system_format == "in user" — merge system into first user message
    merged_system_into_user = False
    formatted_messages = []
    for msg in messages:
        if msg["role"] == "system":
            if system_format == "in user":
                continue  # Will be prepended to first user message
            elif system_format == "string":
            ...
            if system_format == "in user" and not merged_system_into_user:
    ...
    print(f"[ComfyUI Dual VLLM] Calling API: {endpoint}")
    print(f"[ComfyUI Dual VLLM] Model: {model}, Temperature: {temperature}, Max tokens: {max_tokens}")
    ...
    try:
        ...
        if 'text/event-stream' in content_type or 'stream' in content_type:
            ...
            print("[ComfyUI Dual VLLM] Detected streaming response, collecting chunks...")
            full_content = ""
            ...
            if full_content:
                print(f"[ComfyUI Dual VLLM] ✓ Success! Collected {len(full_content)} characters from stream")
            ...
            if "choices" in response_json and len(response_json["choices"]) > 0:
                ...
                print(f"[ComfyUI Dual VLLM] ✓ Success! Tokens: {prompt_tokens} + {completion_tokens} = {total_tokens}")
...
# ═══════════════════════════════════════════════════════════════════════════
# COMFYUI NODE (Dual VLLM - Vision + LLM in one pipeline)
# ══════════════════════════════════════════════════════════════════════════
class ComfyUI_DualVLLM(ComfyNodeABC):
    """ComfyUI Dual VLLM: Two-stage Vision → LLM pipeline.
    
    Dual Mode: Vision model analyzes image → Text model processes your request.
    Single Mode: One multimodal model handles both image and text.
    Use for any task: prompt generation, image analysis, Q&A, etc.
    """

    DESCRIPTION = "Dual VLLM: Vision → LLM pipeline. Dual Mode (Vision + Text) or Single Mode (Multimodal). Any task, not just prompts."
    CATEGORY = "LLM"
    ...
    def INPUT_TYPES(cls) -> InputTypeDict:
        ...
        return {
            "required": {
                ...
                "system_format": (["string", "array", "in user"], {
                    "default": "in user",
                }),
    ...
    def execute(
        ...
        if image is not None:
            try:
                print("[ComfyUI Dual VLLM] Processing image input...")
                ...
                print(f"[ComfyUI Dual VLLM] Image ready: {pil_image.size[0]}x{pil_image.size[1]}")
            except Exception as e:
            ...
            print("[ComfyUI Dual VLLM] Running in text-only mode (no image provided)")
        ...
        if is_single_mode:
            ...
            print(f"[ComfyUI Dual VLLM] {vision_status}")

            # Run Text model (with image if available)
            if text_enable:
                print("[ComfyUI Dual VLLM] Running Text model in Single Mode...")
                ...
                if text_error:
                    ...
                    print(f"[ComfyUI Dual VLLM] Text model error: {text_error}")
                    final_prompt = prompt
                    ...
                    print(f"[ComfyUI Dual VLLM] Text model success: {len(final_prompt)} chars")
                else:
                    text_status = "Text model: Empty response"
                    print("[ComfyUI Dual VLLM] Text model returned empty response")
                    final_prompt = prompt
            else:
                text_status = "Text model: Disabled"
                print("[ComfyUI Dual VLLM] Text model disabled")
                final_prompt = prompt

            print(f"[ComfyUI Dual VLLM] Final prompt length: {len(final_prompt)} characters")
        ...
        if image is not None and vision_enable:
            print("[ComfyUI Dual VLLM] Stage 1: Running vision model...")
            ...
            if vision_error:
                ...
                print(f"[ComfyUI Dual VLLM] Vision model error: {vision_error}")
                vision_description = ""
                ...
                print(f"[ComfyUI Dual VLLM] Vision model success: {len(vision_description)} chars")
            else:
                vision_status = "Vision model: Empty response"
                print("[ComfyUI Dual VLLM] Vision model returned empty response")
                vision_description = ""
        elif image is None and vision_enable:
            vision_status = "Vision model: Skipped (no image provided)"
            print("[ComfyUI Dual VLLM] Vision model skipped: no image")
        else:
            vision_status = "Vision model: Disabled"
            print("[ComfyUI Dual VLLM] Vision model disabled")

        # STAGE 2: Text Model Processing (if enabled)
        if text_enable:
            print("[ComfyUI Dual VLLM] Stage 2: Running text model...")
            text_status = "Text model: Processing..."
...
# ══════════════════════════════════════════════════════════════════════════
NODE_CLASS_MAPPINGS = {
    "ComfyUI_DualVLLM": ComfyUI_DualVLLM
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "ComfyUI_DualVLLM": "ComfyUI Dual VLLM"
}

print("[ComfyUI Dual VLLM] Node registered successfully!")